...
class HexAgent extends Agent {
    ...
    send() {
        ...
        return minMax(board, this.getID()).move;
    }
}

module.exports = HexAgent;

},{"./getEmptyHex":11,"./minMax":12,"ai-agents":4}],1:[function(require,module,exports){
...
function boardScore(board, player) {
    let path0 = rutaFinal(board, player);
    let score = 0;
    if (!path0) {
        score = -(board.length * board.length);
    } else {
        if (path0.length === 2) {
            score = (board.length * board.lensgth);
        } else {
            let path1 = rutaFinal(transpose(board), player);
            if (!path1) {
                score = (board.length * board.length)
            } else {
...
//utiliza dijkstra para encontrar el camino más corto
function rutaCorta(board, player){
    ...
    for(let i = 0; i<cloneBoard.length-1; i++){
      for(let j = 0; j<cloneBoard.length; j++){
        if (board [i][j] !== 0){
          cloneBoard [i][j] = 49999
        }
        if (board[i][j] == player){
          cloneBoard [i][j] = -0.15
        }
    ...
    for(let i = 0; i<cloneBoard.length-1; i++){
      for(let j = 0; j<cloneBoard.length; j++){
        if(j===0){
          nodo = `${i}, ${j+1}`
          inicio_dict[nodo] = cloneBoard [i][j+1]+1
        }
        else if(j===cloneBoard.length-1){
          key = `${i}, ${j-1}`
          route.addNode(key , {'final':1})
        } else {
          nodo = `${i}, ${j}`
          let dict = {}
          
          if(i > 0){
            let arriba = `${i-1}, ${j}` //3,2'
            let arribaDerecha = `${i-1}, ${j+1}`
            dict[arriba] = cloneBoard[i-1][j]+1;
            dict[arribaDerecha] = cloneBoard[i-1][j]+1;
          }
          let izquierda = `${i}, ${j-1}`
          let derecha = `${i}, ${j+1}`
          let abajoIzquierda = `${i+1}, ${j-1}`
          let abajo = `${i+1}, ${j}`
          dict[izquierda] = cloneBoard[i][j-1]+1;
          dict[derecha] = cloneBoard[i][j+1]+1;
          dict[abajoIzquierda] = cloneBoard[i+1][j-1]+1;
          dict[abajo] = cloneBoard[i+1][j]+1;
          route.addNode(nodo , dict)
        }
      }
  
      route.addNode('inicio', inicio_dict)
    }
   
    return route.path('inicio', 'final')
  }

//retorna la ruta final más corta entre dos puntos
function rutaFinal(board, player){
    ruta = rutaCorta(board,player)
    rutaInicio = ruta[1]
    rutaFin = ruta[ruta.length-2]
    
    posActualInicio = [parseInt(rutaInicio[0]),parseInt(rutaInicio[3])]
    posActualFinal = [parseInt(rutaFin[0]),parseInt(rutaFin[3])]
  
    let izquierda = [posActualInicio[0],posActualInicio[1]-1]//2,0
    let abajoIzquierda = [posActualInicio[0]+1,posActualInicio[1]-1]//3,0
    
    let derecha = [posActualFinal[0],posActualFinal[1]+1]
  
    let arribaDerecha = [posActualFinal[0]-1,posActualFinal[1]+1]
    
    if(board[izquierda[0]][izquierda[1]]===0){
        num = convertirAString(izquierda)
        ruta[0] = num
    }else if(abajoIzquierda[0] < 6){
      if(board[abajoIzquierda[0]][abajoIzquierda[1]]===0){
        num = convertirAString(abajoIzquierda)
        ruta[0] = num
      }
    }
  
    if(board[derecha[0]][derecha[1]]===0){
      num = convertirAString(derecha)
      ruta[ruta.length-1] = num
    }else if(arribaDerecha[0]>0){
      if(board[arribaDerecha[0]][arribaDerecha[1]]===0){
        num = convertirAString(arribaDerecha)
        ruta[ruta.length-1] = num
      }
    }
    return ruta
...
function convertirAString(lista){
    num = ""
        for(let i = 0 ; i < lista.length; i++){
          if(i === 0 ){
            num += lista[i].toString()
          }else{
            num += ", "+ lista[i].toString()
          }       
        }
...
const getEmptyHex = require('./getEmptyHex');
const rutaFinal = require('./boardScore');
...
//intentamos  implementar la función minmax pero nos dios muchos errores con el core y las funciones que implementamos para el boardScore con el dijkstra, para que pueda competir improvisamos algo rápido
function minMax(board, player0) {

    //camino = rutaFinal(board, player) --< aiuda
  
    i = Math.floor(Math.random() * player0.length);
    j = Math.floor(Math.random() * player0.length);
    move = [i,j]
