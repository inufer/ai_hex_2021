    ...
    if (!path0) {
        ...
        if (path0.length === 2) {
            ...
            if (!path1) {
                ...
                score = path1.length + ((board.length * board.length) - path0.length);
            }
        }
    }
...
const transpose = require('./transposeHex');
const {boardScore, boardPath} = require('./boardScore');
const getEmptyHex = require('./getEmptyHex');
...
function minMax(board, player0, player, level, maxLevel, alpha, beta, cache, available) {
  ...
  for (let i = 0; i < maxActions; i++) {
    if (available[i] >= 0) {
      ...
      if(level !== maxLevel-1 && i !== 0 && level > 1){
        if (level % 2 == 0) {
          if(score > alpha){
            i = maxActions;
            break;
...
function getCandidates(board) {
  ...
  if(path !== null){
    ...
    path.forEach(x => {
      ...
      let col = x % size;
      let value = boardT[row][col]
      changeIfNeeded(boardT, row, col, value+1);
      pathArr.push(x)
    })
    ...
    path.forEach(y => {
      ...
      let col = y % size;
      board[row][col] = '2'
      if(boardPath(board) === null){
        board[row][col] = 0
        return
      }
      if(boardPath(board).length === pathLength){
        let path1 = boardPath(board)
        path1.splice(0,1)
        path1.splice(path1.length-1,1)
        path1.forEach(z => {
          if(pathArr.includes(z)){
            return
          }
          let row1 = Math.floor(z / size);
          let col1 = z % size;
          let value = boardT[row1][col1]
          changeIfNeeded(boardT, row1, col1, value+1);
          pathArr.push(z)
        })
      }
      board[row][col] = 0
  ...
  let path2 = boardPath(transpose(board))
  if(path2 !== null){
    ...
    path2.forEach(y => {
      ...
      if(boardPath(transpose(board)).length === pathLength){
        let path3 = boardPath(transpose(board))
        path3.splice(0,1)
        path3.splice(path3.length-1,1)
        path3.forEach(z => {
          if(pathArr.includes(z)){
            return
          }
          let col1 = Math.floor(z / size);
          let row1 = z % size;
          let value = boardT[row1][col1];
          changeIfNeeded(boardT, row1, col1, value+1);
          pathArr.push(z)
